...
def setOpened():
    ...
    db.opened.update_one(
        {"roll_no": roll_no.lower()}, {"$set": {"posts": opened_posts}}
    )
    return "success"
...
def sendReplyNotificationToCreator(tokens, name, reply, post_id):
    ...
    message = messaging.MulticastMessage(
        ...
        data={"post_id": post_id, "type": "reply"},
        tokens=valid_tokens,
    )
...
def logout():
    ...
    db.users.update_one(
        {"roll_no": roll_no.lower()}, {"$set": {"fcm_token": fcm_token_list}}
    )
    return "success"
...
def addReply():
    ...
    db.replies.insert_one(
        {
            "user_details": {"roll_no": roll_no.lower(), "name": name, "pfp": pfp},
            "reply": reply,
            "post_id": post_id,
            "date": now.strftime("%I:%M %p | %d %b"),
        }
    )

    roll_no_creator = db.posts.find_one({"_id": ObjectId(post_id)})["user_details"][
        "roll_no"
    ]
    name_creator = db.users.find_one({"roll_no": roll_no_creator})["name"]
    creator_fcm_token_list = db.users.find_one({"roll_no": roll_no_creator})[
        "fcm_token"
    ]
    ...
    db.posts.update_one(
        {"_id": ObjectId(post_id)}, {"$set": {"replies": replies_fcm_token_list}}
    )
...
def getAllReplies():
    ...
    all_posts = [
        str(post["_id"])
        for post in db.posts.find({"user_details.roll_no": roll_no.lower()})
    ]
    all_replies = []
...
def updateProfile():
    ...
    pfp = json["pfp"]
    db.users.update_one(
        {"roll_no": roll_no.lower()}, {"$set": {"name": name, "pfp": pfp}}
    )
    return "success"
...
def login():
    ...
    if user is None:
        ...
        if user["google_user"]:
            return "google_login"
        if sha256_crypt.verify(password, pwd_hash):
            fcm_token_list = db.users.find_one({"roll_no": roll_no.lower()})[
                "fcm_token"
            ]
...
def createPost():
    ...
    post_id = str(
        db.posts.find_one(
            {
                "user_details.roll_no": roll_no.lower(),
                "subject": subject,
                "content": content,
            }
        )["_id"]
